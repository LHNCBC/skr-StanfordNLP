 ...
 *
 */
public class CoreNLPStanford392 {
    private static Logger log = Logger.getLogger(CoreNLPStanford392.class.getName());

    private static CoreNLPStanford392 coreNLP = null;
    private static StanfordCoreNLP pipeline = null;
    private static TreebankLanguagePack tlp = new PennTreebankLanguagePack();

    private CoreNLPStanford392(Properties props) {
	pipeline = new StanfordCoreNLP(props);
    }

    private CoreNLPStanford392() {
     ...
     */
    public static CoreNLPStanford392 getInstance() {
	if (pipeline == null) {
	    log.info("Initializing a Stanford Core NLP instance with default annotators...");
	    coreNLP = new CoreNLPStanford392();
	}
	return coreNLP;
    }
     ...
     * @return a CoreNLP instance
     */
    public static CoreNLPStanford392 getInstance(Properties props) {
	if (pipeline == null) {
	    log.info("Initializing a Stanford Core NLP instance...");
	    coreNLP = new CoreNLPStanford392(props);
	}
	return coreNLP;
    }
	...
	CoreNLPStanford392 cnp = CoreNLPStanford392.getInstance();
	...
	try {
		...
		Thread t = new SocketHandler392(socket, bis, bos, cnp);
		t.start();
		 ...
		 * 
		 * while(true) { try { line = br.readLine(); if(!line.equals("quit")) { if(line
		 * != null) { System.out.println("Input: " + line); String result =
		 * cnp.coreNLP(line, true); System.out.println("Output: " + result);
		 * bw.println(result); bw.flush(); } else continue; } else break; }
		 * catch(Exception e) {
		 * 
		 * } }
		 */
